...
func (c App) GoogleOAuth() revel.Result {
	return c.Render()
}
func (c App) GoogleOAuthCallback() revel.Result {
	Key, _ := revel.Config.String("auth.oauth.google.key")
	gomniauth.SetSecurityKey(Key)
	GoogleOAuthKey, _ := revel.Config.String("auth.oauth.google.id")
	GoogleOAuthSecret, _ := revel.Config.String("auth.oauth.google.secret")
	CallbackURL, _ := revel.Config.String("auth.oauth.google.callback")
	gomniauth.WithProviders(
		google.New(GoogleOAuthKey,
			GoogleOAuthSecret,
			CallbackURL),
	)
	...
	result := creds.Get("access_token").String()
	status = "Successful"
	return c.Render(result, status)
}

func (c App) GoogleOAuth_Login(jwt string) revel.Result {
	Key, _ := revel.Config.String("auth.oauth.google.key")
	gomniauth.SetSecurityKey(Key)
	GoogleOAuthKey, _ := revel.Config.String("auth.oauth.google.id")
	GoogleOAuthSecret, _ := revel.Config.String("auth.oauth.google.secret")
